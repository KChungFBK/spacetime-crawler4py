...
def is_valid(url):
    ...
    try:
        ...
        trap_patterns = [
            ...
            "events/", "/tribe/", "ical", "wics", "~eppstein/pix",
            "fano.ics.uci.edu/rules", "calendar", "filter=", "sort=",
            "offset=", "format=", "replytocom", "utm_", "share=",
            "action=diff", "action=history", "version=", "from=",
            "precision-second", "session", "token", "login", "signup"
        ]
        if any(tp in url.lower() for tp in trap_patterns):
            return False

        if len(url) > 200:
            return False

        if re.search(r"/events/|/~eppstein/pix|/doku.php/", parsed.path.lower()):
            return False
